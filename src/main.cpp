...
EulerAngles toEuler( Quaternion q)
{
    ...
    ret.yaw = atan2(2.0*(q.x*q.y+q.z*q.w),(sqx-sqy-sqz+sqw)) * RAD2DEG;
    ret.pitch = atan2(2.0*(q.y*q.z+q.x*q.w),(-sqx-sqy+sqz+sqw)) * RAD2DEG;
...
void loop()
{
  ...
  if ((myICM.status == ICM_20948_Stat_Ok) || (myICM.status == ICM_20948_Stat_FIFOMoreDataAvail)) // Was valid data available?
  {
    ...
    if ((data.header & DMP_header_bitmap_Quat9) > 0) // We have asked for orientation data so we should receive Quat9
    {
      ...
      SERIAL_PORT.print(eulerAngles.pitch, 3);
      SERIAL_PORT.print(F(" Roll:"));
      SERIAL_PORT.print(eulerAngles.roll, 3);
      SERIAL_PORT.print(F(" Yaw:"));
      SERIAL_PORT.print(eulerAngles.yaw, 3);
      SERIAL_PORT.print(F(" test:"));
      SERIAL_PORT.print(q1*q2+q3*q0, 3);
